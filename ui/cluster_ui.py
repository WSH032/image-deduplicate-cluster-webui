    ...
    with gr.Blocks(title=blocks_name, css=css) as cluster_ui:
        ...
        with gr.Box():
            ...
            with gr.Row():
                ...
                # 注意，这里的visible=True请按feature_extraction_method_Radio的默认值来设置
                with gr.Box(visible=True) as text_feature_extraction_Box:
                    gr.Markdown("**请用 '聚类特征选择' 切换此Box**")
                    text_vectorizer_method = gr.Dropdown(
                ...
                # 注意，这里的visible=True请按feature_extraction_method_Radio的默认值来设置
                with gr.Box(visible=False) as image_feature_extraction_Box:
                    gr.Markdown("**请用 '聚类特征选择' 切换此Box**")
                    wd14_feature_layer_choice = gr.Dropdown(
